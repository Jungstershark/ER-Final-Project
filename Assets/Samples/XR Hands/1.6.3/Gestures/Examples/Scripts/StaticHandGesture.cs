...
namespace UnityEngine.XR.Hands.Samples.GestureSample
{
    ...
    public class StaticHandGesture : MonoBehaviour
    {
        ...
        void OnJointsUpdated(XRHandJointsUpdatedEventArgs eventArgs)
        {
            ...
            {
                Debug.Log($"[Gesture Debug] Started detecting shape on: {gameObject.name}");
                m_HoldStartTime = Time.timeSinceLevelLoad;
            }
            else if (m_WasDetected && !detected)
            {
                if (!m_PerformedTriggered)
                {
                    Debug.Log($"[Gesture Debug] Lost shape before hold time complete on: {gameObject.name}");
                }
            ...
            if (!m_PerformedTriggered && detected)
            {
                ...
                {
                    // DEBUG 3: The big success log
                    Debug.Log($"[Gesture Debug] <color=green>GESTURE PERFORMED</color> on: {gameObject.name}");
